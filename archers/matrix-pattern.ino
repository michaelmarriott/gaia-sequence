...
void MatrixUpdate() {
 for (int i = 0; i < NUM_LEDS_PER_STRIP; i++) {
    for (int j = 0; j < NUM_STRIPS; j++) {
      byte layer = matrix_rain[XYs( ((i + matrix_speed + random8(2) + NUM_LEDS_PER_STRIP) % NUM_LEDS_PER_STRIP),j)];   //fake scroll based on shift coordinate
      // random8(2) add glitchy look
      if (layer) {
        leds[XYs((NUM_LEDS_PER_STRIP - 1) - i, (NUM_STRIPS - 1) - j)] = matrix_color;
      }
    }
  }
