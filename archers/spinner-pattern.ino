  ...
  for (int x = 0; x < NUM_STRIPS; x++) {
    if (showIt(spin_top, x, 5)) {
      for (int i = NUM_LEDS_PER_STRIP_SPLIT; i < NUM_LEDS_PER_STRIP; i++) {
        leds[(x * NUM_LEDS_PER_STRIP) + i] = spin_top_colour;
      }
    } else {
      for (int i = NUM_LEDS_PER_STRIP_SPLIT; i < NUM_LEDS_PER_STRIP; i++) {
        leds[(x * NUM_LEDS_PER_STRIP) + i] = spin_top_colour_fade;
      }
    }
  }
  ...
  for (int x = NUM_STRIPS - 1; x >= 0; x--) {
    ...
    if (showIt(spin_bottom, x, 3)) { //} {
      for (int i = 0; i < NUM_LEDS_PER_STRIP_SPLIT; i++) {
        leds[(x * NUM_LEDS_PER_STRIP) + i] = spin_bottom_colour;
      }
    } else {
      for (int i = 0; i < NUM_LEDS_PER_STRIP_SPLIT; i++) {
        leds[(x * NUM_LEDS_PER_STRIP) + i] = spin_bottom_colour_fade;
      }
    }
  }
